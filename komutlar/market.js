...
exports.run = async (client, message, args) => {
  ...
  let ewing = db.fetch(`pusula_${message.author.id}`)

  const embed = new Discord.MessageEmbed()
  .setTitle(`Pusula Marketi`)
  .setColor("#F0FFFF")
  .setFooter(`${message.author.tag} ● Sahip olunan para: ${para || "0"}TL ● Sahip olunan Pusula: ${ewing || "0"} `)
...
let para = db.fetch(`para_${message.author.id}`)
    let miktar = args[1];
    if(!miktar) return message.channel.send(` Alacağın pusulayı belirtmeden pusula alamazsın.`)

    if(!miktar > para) return message.channel.send(` Alacağın pusula, parandan daha fazla olamaz.`)
    if(para < miktar*100 ) return message.channel.send(` Pusula almak için yeterli paran yok.`)
...
if(args[0] == "pusula-sat") {
  let ewing = db.fetch(`pusula_${message.author.id}`)
  let miktar = args[1];
  if(!miktar) return message.channel.send(` Satacağın pusulayı belirtmeden pusula alamazsın.`)
  if(!miktar > pusula) return message.channel.send(` Belirttiğin miktar kadar pusulan yok.`)
...
if(args[0] == "oy-verme") {
  ...
  if(oyşeysi) return message.channel.send(` Zaten Premium üyesin.`)
